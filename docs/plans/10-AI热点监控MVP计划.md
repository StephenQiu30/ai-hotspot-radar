...
- 已实现：统一 `Candidate` 候选结构、多来源 adapter、AI 查询扩展、本地分析降级、`active`/`filtered` 阈值过滤、事件邮件通知、`/api/search`、`/api/reports` 日报/周报。
- 需配置后实测：X/Twitter、Bing、SMTP 真实发送和 OpenAI 兼容模型调用。
- 暂不做：实时推送、多租户、真实登录认证、计费、复杂队列、向量库和复杂报告模板系统。
...
- 低相关热点不会发送事件邮件，也不会进入日报/周报。
- 高相关热点可以进入热点列表、事件邮件和日报/周报。

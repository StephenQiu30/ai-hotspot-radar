...
- 已实现：`POST /api/check-runs` 检测链路、`POST /api/search` 即时搜索、`/api/reports` 日报/周报、`active`/`filtered` 阈值过滤、报告只聚合 `active` 热点、SMTP 缺失降级和后端测试用例。
- 需配置后实测：X/Twitter、Bing、SMTP 真实发送和 OpenAI 兼容模型真实调用。
- 当前报告内容以本地 Markdown 模板为 P0 必需能力；AI 报告增强不作为本阶段验收前提。
...
12. 报告内容使用本地 Markdown 模板生成。
13. 后续可增加 AI 报告增强；AI 未配置或失败时不得阻塞模板报告。
...
- 报告生成必须优先使用模板报告；后续 AI 增强失败时也必须回退到模板报告。
